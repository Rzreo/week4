...
import com.sparta.jwtprac.controller.dto.TokenDto;
...
public class TokenProvider {//토큰 만듬
    ...
    public TokenDto generateTokenDto(Authentication authentication) {//토큰 만들기
        // 권한들 가져오기
        String authorities = authentication.getAuthorities().stream()
                .map(GrantedAuthority::getAuthority)
                .collect(Collectors.joining(","));//하나의 문자열로 joining
        ...
        String accessToken = Jwts.builder()
                .setSubject(authentication.getName())       // payload "sub": "name"//id가 들어감
    ...
    public boolean validateTokenRequestDto(TokenRequestDto requestDto, HttpServletResponse response) {
        try {
            Jwts.parserBuilder().setSigningKey(key).build().parseClaimsJws(requestDto.getAccessToken());
            ...
            log.info("만료된 JWT 토큰입니다.");
            return authService.reissue(requestDto, response);
        } catch (UnsupportedJwtException e) {
            log.info("지원되지 않는 JWT 토큰입니다.");
        } catch (IllegalArgumentException e) {
            log.info("JWT 토큰이 잘못되었습니다.");
